    ...
    def __init__(self,
        ...
        with tf.variable_scope(scope, default_name="bert"):
            ...
            with tf.variable_scope("encoder"):
                ...
                self.all_encoder_layers = transformer_model_theseus(
                    input_tensor=self.embedding_output,
                    ...
                    initializer_range=config.initializer_range,
                    do_return_all_layers=True,replace_rate_prob=replace_rate_prob,succ_layers=suc_layers,is_training=is_training,finetune_suc=finetune_suc)
...
def fetch_from_single_line(self,search_pattern, str):
        if not re.search(search_pattern, str):
            return ""
        else:
            group = re.search(search_pattern, str).groups()
            return group[0]
...
def get_assignment_map_from_checkpoint_for_theseus(tvars, init_checkpoint,suc_layer_prefix="suc_"):
    """
    为theseus-bert 压缩特制，要将替换的layer预先用原始的layer权重加载，用前n层来直接代替 bert/encoder/suc_layer_0
    :param tvars: 当前模型图中的所有变量
    :param init_checkpoint: bert预训练模型中的变量图
    :return:
    """
    """Compute the union of the current variables and checkpoint variables."""
    assignment_map = {}
    ...
    # with open("all_var", 'w') as fw:
    #     for key in name_to_variable.keys():
    #         fw.write(key)
    #         fw.write("\n")
    # with open("not_include_var", 'w') as fw:
    for x in init_vars:
            (name, var) = (x[0], x[1])
            if name not in name_to_variable:
                # fw.write(name)
    ...
    return (assignment_map,suc_assignment_map,initialized_variable_names)
...
def transformer_model_theseus(input_tensor,
                      attention_mask=None,
                      hidden_size=768,
                      num_hidden_layers=12,
                      num_attention_heads=12,
                      ...
                      attention_probs_dropout_prob=0.1,
                      initializer_range=0.02,
                      do_return_all_layers=False,replace_rate_prob=0.5,succ_layers=6,is_training=True,finetune_suc=False):
    ...
    Args:
      ...
      do_return_all_layers: Whether to also return all layers or just the final
        layer.
      replace_rate_prob: replace probability for Successor module replacing
      succ_layers: successor module numbers
      is_training: whether is training
      finetune_suc: phase of bert-of-theseus. True for final phase of successor module finetuning;False for model replacing
    ...
    if do_return_all_layers:
        ...
        # 目前不关注pre的所有layer，只要在inference时返回所有suclayer就可以了。
...
def get_replace_prob_stratege(base_replace_prob,k,global_step,strategy):
    """

    :param base_replace_prob:
    :param k:
    :param global_step:
    :param strategy: constant；linear
    :return:
    """
